...
const page = () => {
    const columns = ["Action", '_id', 'Package Name', 'Slug', 'Created at', "Last Date of Registration"];
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const res = await fetch(
                    `${BASE_URL}/api/packages?page=${currentPage}|| ""}`,
                    {
                        cache: "no-store",
                    }
                );
        ...
        loadData().then((data) => {
            ...
            setData(data.packages.map((d) => [
                ...
                ,
                d._id, d.packageName, d.slug, new Date(d.createdAt).toDateString(), new Date(d.lastDate).toDateString()]));
